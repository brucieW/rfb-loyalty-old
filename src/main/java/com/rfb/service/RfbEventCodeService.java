...
//        List<RfbLocation> locations = rfbLocationRepository.findAllByRunDayOfWeek(LocalDate.now().getDayOfWeek().getValue());
        List<RfbLocation> locations = rfbLocationRepository.findAllByRunDayOfWeek(1);

        log.debug("Locations Found for Events: " + locations.size());
        ...
        locations.forEach(location -> {
            ...
            if (existingEvent == null) {
                ...
                newEvent.setEventCode(RandomStringUtils.randomAlphanumeric(5).toUpperCase());
